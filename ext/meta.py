...
class Meta:
    ...
    async def _change_game(self, *, game_chosen: str):
        ...
        try:
            await self.bot.change_status(game=game_str, idle=False)
            await self.bot.say("```Game correctly changed to {0}```".format(game_chosen))
        except discord.HTTPException:
            await self.bot.say("```Could not change game, please try again```")
